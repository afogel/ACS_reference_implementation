...
Every generated file takes `--check`, which exits 1 when the committed copy is stale. CI runs the census, markers, extract, render and lint checks on every push; `spec-lint.yml` additionally lints each pull request against its base branch and posts the normative-impact comment.
...
| `src/render/` | code | N52 the census report, N50 the provision index, U9 the lint report, U10/U32 the stale list and worklist, N27 the impact comment. |
...
| V5 to V8 | planned | Compiled predicates with the differential oracle, the conformance report, the full conversion, upstreaming. |
