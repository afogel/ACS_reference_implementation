...
export function renderStale(report: StalenessReport): string {
  const out = ["## Provisions needing review (U10)", ""];
...
function describe(reason: StaleReason): string {
  switch (reason.kind) {
    case "text_changed":
      return `its text changed: reviewed against \`${reason.reviewed_against.slice(0, 12)}\`, now \`${reason.text_hash.slice(0, 12)}\` (R2.4)`;
    case "dependency_stale":
      return `it depends on ${reason.via}, which needs review (R2.6)`;
    case "restatement_diverged":
      return `it restates ${reason.canonical}, whose canonical text changed; the concept page wins (R2.9)`;
  }
}

export function renderWorklist(worklist: WorklistEntry[]): string[] {
  const lines = ["## Migration worklist (U32)", ""];
