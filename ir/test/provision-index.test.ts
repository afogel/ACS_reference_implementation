...
describe("renderProvisionIndex -- P2 from the real catalog", () => {
  ...
  const index = renderProvisionIndex(catalog, manifest.corpus);

  it("renders the index (U12), a detail per provision (U13), and the coverage table (U14)", () => {
    expect(index).toContain("| [ACS-REQ-0007](#acs-req-0007) | Requirement | — | observed-agent | acs-core | wire | obligation | active | spec/instrument/specification.md:65 | Handshake precedes any hook traffic |");
    ...
    expect(index).toContain("| ACS-EXC-0001 | none yet (V5) |");
