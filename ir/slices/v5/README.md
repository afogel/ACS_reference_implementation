...
| ACS-REQ-0003 required fields | a static relation with seven tuples standing in for the §6 table |
...
**Inexpressible count: zero, this time.** The vocabulary derived from these specimens expressed all of them. V7 is where `flagInexpressible()` meets the other 180 occurrences, and a non-empty count there is expected.

## Decisions made here

- **Soufflé is an oracle and a deliverable, never a local dependency (X4, R3.9).** `bun install` is the laptop path. CI installs the pinned 2.5 `.deb` (SHA-256 verified) and runs both engines; locally, `SOUFFLE=/path/to/souffle bun run ir differential` does the same, and the test that needs the oracle says it was not exercised rather than passing vacuously.
- **`count` counts distinct solutions of the aggregate body**, as Soufflé does; a wildcard column contributes each tuple.
- **The recursive lineage closure assumes a DAG.** A `derived_from` cycle would not terminate in either engine's path-carrying form; a cycle is itself a defect a future rule can name.
...
- The test-coverage table (U14) is now real: 24 of 28 provisions are cited by a fixture; N25 lints the same citations.
...
No conformance report, no profile scoping, no exclusion roster. The verdicts exist as tuples; V6 gives them a reader.
