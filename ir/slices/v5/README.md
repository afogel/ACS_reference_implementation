...
**The language ([`ir/src/compile/predicate.ts`](../../src/compile/predicate.ts)).** Datalog rules as strings in the record's `predicate` block: atoms, `not`, the six comparators, `Var = expr` assignment, `cat` and `to_string`, and `count`. A `violation(subject..., witness...)` head plus helper relations; a head position may hold a constant, which is how a rule names the field it found missing. That is the entire language, because every construct is emitted twice and held equivalent. A construct only one engine had would be a construct nobody had checked.

**The compiler ([`ir/src/compile/compile.ts`](../../src/compile/compile.ts)).** Types every variable against the vocabulary, including its domain (a variable bound to a `seq` column cannot fill a `bound` column, and two domains cannot be compared), infers helper column types to a fixpoint, rejects unsafe rules (a negated or compared variable nothing binds), stratifies by SCC and refuses negation inside a cycle, and enforces N36: the violation head is the declared subject then witness variables in that order, each position its variable or a constant, both lists non-empty, all bound. A Requirement with neither a predicate nor a declared `inexpressible` reason does not compile (R3.1, R3.6). Helpers are renamed per provision so twenty-one predicates share one program without collision.

**Two targets, one authority (R3.3, R3.8).** `emitSouffleProgram()` writes the `.dl` an auditor runs with stock Soufflé over a directory of `.facts`: each column domain is declared as a Soufflé subtype, and each compiled provision's violation relation is an `.output` with typed subject and witness columns. `rules.json` is the same typed program for the evaluator, which is a semi-naive fixpoint per stratum. `differentialCheck()` reads Soufflé's per-provision CSVs back into the one-line form the fixtures pin in `expected.tsv` (provision, subject values, witness values) and compares it with the evaluator's, so two engines cannot agree on a wrong answer.
...
| ACS-REQ-0010 trust monotonicity | a recursive transitive closure over `derived_from`, the static `trust_rank` table, and `>` over ranks |
...
- **The recursive lineage closure is over (descendant, ancestor) pairs**, so a `derived_from` cycle terminates. The first version built the path string inside the recursion; that does not terminate on a cycle in either engine, and a trace is untrusted input. The report's evidence lists the `derived_from` facts for the subject instead.
- **The Soufflé program is written the way a Soufflé author would write it.** Spec-fixed tables are facts (`default_trust("user_input", "trusted").`), constants sit in the head, each column domain is a declared subtype, and each provision has its own typed `.output`. The first version packed every provision's columns into one three-column relation with `cat`; the fixtures still use that one-line form, and the differential reads Soufflé's CSVs into it.
