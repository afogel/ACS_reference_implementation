...
The closing comment survives the build as a comment. Stripping tags and comments from the rendered HTML leaves no `<a id`, no `<!--/`, and no `acs-req-` anywhere in the visible text. The spec repository's own guards (`tests/test_spec_structure.py`, `tests/test_hook_taxonomy.py`) pass on the patched tree.
...
- **Terminators are HTML comments, and that is fine for MkDocs.** Python-Markdown passes inline HTML and comments through; the built page keeps them as comments. A renderer that strips comments would lose the terminator but not the anchor, and the deep links would still work.
...
3. On merge, open the bulk PR from `ir/dist/markers.patch` (U30). Ariel, from `bulk-pr.md`. If `main` has moved, bump the submodule first and regenerate; `markers apply` names any quote that no longer resolves and any entry already marked.
