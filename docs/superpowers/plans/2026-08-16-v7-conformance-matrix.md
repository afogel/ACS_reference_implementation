...
**The five rows already measured in §V7 must come out of this check, not be asserted beside it** — `gen_ai.tool.name` green, `acs.capability` red because `hooks/tool-call-request.json` requires only `tool` and `arguments`, `acs.decision` green, `acs.evaluator` red with no wire source, and the three conditional attributes red because no such fields exist on `AcsResult`.
