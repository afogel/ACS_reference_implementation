...
| # | Risk | Handling |
|---|------|----------|
| 1 | The `mirrors` post-condition's substring test could refuse a *legitimate* replacement that happens to contain the original text | Accepted, fail-closed, and on the same side as every other refusal in `result-output.ts`. Recorded in §V5 so V7's matrix can carry it |
