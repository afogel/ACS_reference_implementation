...
2. **The audit sink is total, exactly as the envelope tap is (N26's constraint 8, now S14's).** It never throws, never delays a decision, never changes one. ~~A sink that cannot write degrades observability and nothing else.~~ **⚠️ Amended by the whole-branch review.** That last sentence contradicted constraint 3, and the whole-branch review found the contradiction shipped: an unwritable audit path produced an unrecorded fail-open proceed. **Constraint 3 governs** — §6.4 makes the audit entry a MUST for a step that proceeds without a decision, so an unauditable bypass is not one the spec permits. The sink still never throws; what changed is that `write` reports whether the append succeeded, and `applyFailurePosture` downgrades an unauditable `proceed` to `deny`. Totality was what this constraint existed to protect, and totality is intact.
