...
**⚠️ Correction — "new entries in S1" understates it; `renderDecision` cannot express this gate at all.** N3 requires a `permissionDecision` and hardcodes three Claude Code field names (`permissionDecision`, `permissionDecisionReason`, `updatedInput`). `PostToolUse` has none of them, and needs a **top-level** `decision`/`reason` alongside `hookSpecificOutput.updatedToolOutput`. S1's `decisions` block therefore moves **under each hook** and declares host output field names as data (`set` / `from` / `top_level`), with the adapter naming none of them. That is also what makes **V5's `N12` — "`renderDecision()` — same module as `N3`" — literally true**: today OpenCode would have to patch the shared module, so V4 is where a second *hook* forces the generalization a second *host* would have forced anyway.
