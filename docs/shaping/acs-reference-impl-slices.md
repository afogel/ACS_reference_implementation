...
| 14 | ⚠️ When the negotiated ServerHello cannot be *persisted*, the posture it declared is still not applied to the current hook | V3 | Partly closed. The failure is now visible — it lands in the audit entry as `session_failure` instead of being swallowed by a bare `catch {}`, which is what made a deployment declaring `deny` fail silently open. But `handshake()` returns the ServerHello and the shim still discards it when `store.set` throws, so the posture is known in-process and unused for that step. Using the returned value would close it; the whole-branch review's ruling required visibility, and visibility is what shipped |
