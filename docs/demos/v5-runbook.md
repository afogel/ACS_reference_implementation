...
degrades to a log line here rather than changing that it does.
...
- **A third host's `tools` list is scoped on the tool that host says it dispatched, and nothing
  ...
  **Pre-existing, not round 4's doing:** the outcome is identical before and after, because host
  ...
  scheduled harness that diffs two declared surfaces and reports a `SurfaceDiff` naming the field
  that moved — the
