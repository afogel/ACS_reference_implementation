...
⚠️ **Added by the final review round — a URL the *gate* mis-parses, which is worse than one the extractor misses, and only the shell route could be closed.** `egress.rego`'s `host_of()` cuts the post-scheme substring at the first `/` and then at the first `:`, so a userinfo carrying a colon reads as the host: measured, `host_of("https://docs.anthropic.com:pw@exfil.attacker.test/steal")` is `docs.anthropic.com`, and against the shipped allowlist that URL was **allowed on both routes**. The parser is AGT's, in the bundle `verify:pin` holds byte-identical. `annotateEgressDestination` now strips the userinfo from the string it supplies, so the shell route denies; the fetch route reads `args.url` through the gate's own first destination path with nothing in between, and closing it would mean editing a `.rego`. Risk row 26, captured in the runbook's §5.
